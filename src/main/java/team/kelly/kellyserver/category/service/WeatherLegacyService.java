...
import team.kelly.kellyserver.category.dto.WeatherResultLegacyDto;
...
public class WeatherLegacyService {
    ...
    public WeatherResultLegacyDto getOneDayWeatherLegacyData(WeatherSearchDto infoVO) throws IOException {

        GridValueDto gridValueDto = TransLocalPoint.getGridxy(infoVO.getLat(), infoVO.getLon());

        WeatherResultLegacyDto resultDto = getVilageFcstWeatherData(gridValueDto);
    ...
    public WeatherResultLegacyDto getVilageFcstWeatherData(GridValueDto gridValueDto) throws IOException {

        WeatherResultLegacyDto result = new WeatherResultLegacyDto();
    ...
    private JSONArray callWeatherApi(GridValueDto gridValueDto, String base_date, String base_time, String vilageWeatherUrlPrefix, int pageNo) throws IOException {
        String jsonStr = ApiUtility.callApi(vilageWeatherUrlPrefix + govOpenApiKey + "&pageNo=" + pageNo + "&numOfRows=200&dataType=XML&base_date="
                + base_date + "&base_time=" + base_time + "&nx=" + gridValueDto.getGridx() + "&ny=" + gridValueDto.getGridy(), "xml");
